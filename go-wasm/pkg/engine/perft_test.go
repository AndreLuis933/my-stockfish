...
// TestPerftFast validates that PerftFast produces the same node counts as
// the reference Perft implementation across all standard positions.
func TestPerftFast(t *testing.T) {
	cases := []struct {
		fen   string
		depth int
	}{
	...
	for _, tc := range cases {
		Game.reset()
		LoadFen(tc.fen)
		want := Game.Perft(tc.depth)

		Game.reset()
		LoadFen(tc.fen)
		got := Game.PerftFast(tc.depth)
		if got != want {
			t.Errorf("PerftFast(%d) = %d, Perft(%d) = %d (fen: %s)", tc.depth, got, tc.depth, want, tc.fen)
		}
	}
}